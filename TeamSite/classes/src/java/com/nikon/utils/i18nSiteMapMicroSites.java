...
public class i18nSiteMapMicroSites extends LivesiteSiteMap {

	private Log log = LogFactory.getLog(i18nSiteMapMicroSites.class);

    public i18nSiteMapMicroSites() {
    }

      //get cookie to help set up path
    public Document getLangCookie(RequestContext context) {
		...
		Cookie lcookie = context.getCookies().getCookie("langCookie");

		if (lcookie != null) {
			langValue = lcookie.getValue().toString();
		}
		else
		{
			langValue= "en_Asia";
                ...
                response.addElement("LangCookie").addText(langValue);
		return doc;
	}

    public Document getSiteMapSubSite(RequestContext context) {
        ...
        //remove sub site from string to get to parent site map
        String subLoc;
          if (context.getParameterString("SubLoc") == null) {
                subLoc = "";
            } else {
                subLoc = context.getParameterString("SubLoc");
            }

        rootPath = rootPath.replace(subLoc, "");
        ...
        for (Iterator iter = nodeList.iterator(); iter.hasNext();) {
            ...
            while (iterator.hasNext()) {
                ...
                if (childElement.getName().equals("label")) {
                    ...
                    log.debug("keystring is --> ***" + keyString +"***");
