...
import com.interwoven.teamsite.javax.util.Filter;
...
import com.interwoven.teamsite.nikon.hibernate.beans.AwardTest2Award;
import com.interwoven.teamsite.nikon.hibernate.beans.AwardTestimonial;
import com.interwoven.teamsite.nikon.hibernate.beans.NewsArticle;
...
import com.interwoven.teamsite.nikon.hibernate.beans.Product2Award;
...
public class NikonHBN8DAOManagerTxn
extends AbstractTSEnvAwareHibernateDAOManager
implements NikonBusinessManager
{
	...
	static {
		try {
			NikonHBN8DAOManagerTxn.ProductDTOConstructorCache = ProductDTO.class.getConstructor(new Class[] {Product.class});
	 ...
	 */
	public static final NikonHBN8DAOManagerTxn getSpringAppCtxInstance()
	throws BeanDefinitionStoreException
	{
		...
		return (NikonHBN8DAOManagerTxn)o;
	}

	public NikonHBN8DAOManagerTxn()
	{
...
//		MetricLog metric = param.getMetricLog() != null ? param.getMetricLog() :metrics.getMetricLog("listProductDetails");
//		metric.startTiming();

		ProductDTO retVal = null;

		//List the Product first
		//ProductDTO dto = null;

		//Setup the session
//		metric.recordTime("Begining HBN8 Txn");
...
//				metric.recordTime("Enabling HBN8 Filter " + NikonHBN8FilterConstants.WWA_DATE);
				enableDisableFilter(session, NikonHBN8FilterConstants.WWA_DATE, param.isEnableWWAFilter());

				//Set up the query and parameters
				//TODO Constants

//				metric.recordTime("Getting HBN8 Query " + NikonHBN8QueryConstants.SNG_PROD_IN_LOCALE);
				Query singleProductQry = session.getNamedQuery(NikonHBN8QueryConstants.SNG_PROD_IN_LOCALE);
//				metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.SNG_PROD_IN_LOCALE);
...
//				metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.SNG_PROD_IN_LOCALE);
				List<ProductDTO> queryList = (List<ProductDTO>)filterUnwantedLocales(singleProductQry.setResultTransformer(new AliasToBeanConstructorResultTransformer(ProductDTOConstructorCache)).list(), new LocaleComparator(possibleLocales), ProductDTO.class);
//				metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.SNG_PROD_IN_LOCALE);
				...
				if((queryList != null) && (queryList.size() > 0))
				{
					...
					{
						//TODO better error handelling here
						try
						{
...
//					metric.recordTime("Getting HBN8 Query " + NikonHBN8QueryConstants.PRD_2_ACC);
					Query prod2AccessoryQuery = session.getNamedQuery(NikonHBN8QueryConstants.PRD_2_ACC);
//					metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.PRD_2_ACC);
...
//					metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.PRD_2_ACC);
					List<Product2Accessory> prod2AccList = prod2AccessoryQuery.list();
//					metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.PRD_2_ACC);
					...
					List<ProductDTO> marketRelatedProductsReverse = new LinkedList<ProductDTO>();

					//Go get the values
					if(prod2AccList.size() > 0)
					{
//						metric.recordTime("Getting HBN8 Query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						multiProductQry = session.getNamedQuery(NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
//						metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
//						metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
...
//						metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						accessories = (List<ProductDTO>)filterUnwantedLocales(multiProductQry.setResultTransformer(new AliasToBeanConstructorResultTransformer(ProductDTOConstructorCache)).list(), new LocaleComparator(possibleLocales), ProductDTO.class);
//						metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						removeLocaleOptOutProducts(accessories);
						
					}

					//Marketing Related Products
//					metric.recordTime("Getting HBN8 Query " + NikonHBN8QueryConstants.PRD_2_PRD_MARKETING_RELATED);
					Query prod2MarketingRelatedProdQuery = session.getNamedQuery(NikonHBN8QueryConstants.PRD_2_PRD_MARKETING_RELATED);
//					metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.PRD_2_PRD_MARKETING_RELATED);
...
//					metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.PRD_2_PRD_MARKETING_RELATED);
					List<Product2ProductMarketingRelated> prod2MarkPrdList = prod2MarketingRelatedProdQuery.list();
//					metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.PRD_2_PRD_MARKETING_RELATED);
...
//						metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						...
						multiProductQry.setParameterList(NikonHBN8ParamConstants.NKN_LOCALES, possibleLocales);
//						metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						marketRelatedProducts = (List<ProductDTO>)filterUnwantedLocales(multiProductQry.setResultTransformer(new AliasToBeanConstructorResultTransformer(ProductDTOConstructorCache)).list(), new LocaleComparator(possibleLocales), ProductDTO.class);
//						metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);

						removeLocaleOptOutProducts(marketRelatedProducts);
						
//						FormatUtils.pFormat("++++++++++++++++++");
//						for(ProductDTO to: marketRelatedProducts)
//						{
//							FormatUtils.pFormat(to.getProdDevCode());
//						}
//						FormatUtils.pFormat("++++++++++++++++++");
					}
					
					Query prod2MarketingRelatedProdQueryReverse = session.getNamedQuery(NikonHBN8QueryConstants.PRD_MARKETING_RELATED_2_PRD);
//					metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.PRD_2_PRD_MARKETING_RELATED);
					prod2MarketingRelatedProdQueryReverse.setParameter(NikonHBN8ParamConstants.PRODUCT_ID, prod.getProdId());
					prod2MarketingRelatedProdQueryReverse.setParameter(NikonHBN8ParamConstants.NKN_CNTRY_CD, param.getSiteCountryCode());
//					metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.PRD_2_PRD_MARKETING_RELATED);
					List<Product2ProductMarketingRelated> prod2MarkPrdListReverse = prod2MarketingRelatedProdQueryReverse.list();
//					metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.PRD_2_PRD_MARKETING_RELATED);
					log.info(FormatUtils.mFormat("prod2MarkPrdList.size({0})", prod2MarkPrdListReverse.size()));

					//Go get the values
					if(prod2MarkPrdListReverse.size() > 0)
					{
//						metric.recordTime("Getting HBN8 Query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						multiProductQry = session.getNamedQuery(NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
//						metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
//						metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						multiProductQry.setParameterList(NikonHBN8ParamConstants.PRODUCT_IDS, listOfBeansFieldToListOfStrings(prod2MarkPrdListReverse, ProductDTO.fk));
						multiProductQry.setParameterList(NikonHBN8ParamConstants.NKN_LOCALES, possibleLocales);
//						metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						marketRelatedProductsReverse = (List<ProductDTO>)filterUnwantedLocales(multiProductQry.setResultTransformer(new AliasToBeanConstructorResultTransformer(ProductDTOConstructorCache)).list(), new LocaleComparator(possibleLocales), ProductDTO.class);
//						metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);

						removeLocaleOptOutProducts(marketRelatedProductsReverse);
...
//					metric.recordTime("Getting HBN8 Query " + NikonHBN8QueryConstants.PRD_2_PRD);
					Query prod2RelatedProductsQuery = session.getNamedQuery(NikonHBN8QueryConstants.PRD_2_PRD);
//					metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.PRD_2_PRD);
...
//					metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.PRD_2_PRD);
					List<Product2Product> prod2RelatedProdList = prod2RelatedProductsQuery.list();
//					metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.PRD_2_PRD);
...
//						metric.recordTime("Getting HBN8 Query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						multiProductQry = session.getNamedQuery(NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
//						metric.recordTime("Creating Collection of ProdIds");
...
//						metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						...
						multiProductQry.setParameterList(NikonHBN8ParamConstants.NKN_LOCALES, possibleLocales);
//						metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						relatedProducts = (List<ProductDTO>)filterUnwantedLocales(multiProductQry.setResultTransformer(new AliasToBeanConstructorResultTransformer(ProductDTOConstructorCache)).list(), new LocaleComparator(possibleLocales), ProductDTO.class);
//						metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
...
//					metric.recordTime("Getting HBN8 Query " + NikonHBN8QueryConstants.PRD_2_BOM);
					Query prod2BomProductsQuery = session.getNamedQuery(NikonHBN8QueryConstants.PRD_2_BOM);
//					metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.PRD_2_BOM);
...
//					metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.PRD_2_BOM);
					List<Product2Bom> prod2BomProdList = prod2BomProductsQuery.list();
//					metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.PRD_2_BOM);
...
//						metric.recordTime("Setting HBN8 Parameters on query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						...
						multiProductQry.setParameterList(NikonHBN8ParamConstants.NKN_LOCALES, possibleLocales);
//						metric.recordTime("Starting execution of HBN8 query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
						bomProducts = (List<ProductDTO>)filterUnwantedLocales(multiProductQry.setResultTransformer(new AliasToBeanConstructorResultTransformer(ProductDTOConstructorCache)).list(), new LocaleComparator(possibleLocales), ProductDTO.class);
//						metric.recordTime("Finished executing HBN8 query " + NikonHBN8QueryConstants.MLT_PRD_PRD_ID_LOC);
					...
					//If we're a product then add some dummy stuff here
					//Should always look for the reverse 
					if(prod.isAccessory() || true)
					{
						Query accessory2ProductQuery = session.getNamedQuery(NikonHBN8QueryConstants.ACC_2_PRD);
						...
						if(acc2ProdList.size() > 0)
						{
							...
							multiProductQry.setParameterList(NikonHBN8ParamConstants.NKN_LOCALES, possibleLocales);
							accessoryOf = (List<ProductDTO>)filterUnwantedLocales(multiProductQry.setResultTransformer(new AliasToBeanConstructorResultTransformer(ProductDTOConstructorCache)).list(), new LocaleComparator(possibleLocales), ProductDTO.class);
...
//					metric.recordTime("Starting Assembleing Process");
					...
					//TODO this needs to change to the actual
					...
					assembler.setProduct2marketingRelatedProductReverse(prod2MarkPrdListReverse);
					assembler.setMarketingRelatedProductsReverse(marketRelatedProductsReverse);

					retVal = assembler.assembleSingle();
//					metric.recordTime("Ended Assembleing Process");
				}
...
//				metric.stopTiming();
	...
	public ProductDTO listProductMetaDataFromDCRPath(HBN8QueryParamDTO param)
	{
		...
		try
		{
			...
			try
			{
				...
				Query productMetaDataFromPath = session.getNamedQuery(NikonHBN8QueryConstants.PRD_MT_DT_FRM_PTH);
				...
				List<ProductDTO> list = productMetaDataFromPath.setResultTransformer(new AliasToBeanConstructorResultTransformer(ProductDTOConstructorCache)).list();
				if(list.size() == 1)
				{
	...
	private class LocalePriceFilter
	implements Filter
	{
		String locale;
		public LocalePriceFilter(String locale)
		{
			this.locale = locale;
		}

		public boolean test(Object testObject) {
			Price p = (Price)testObject;
			// TODO Auto-generated method stub
			return locale.equals(p.getNikonLocale());
		}
	...
	public List<ProductDTO> listNavCatProducts(HBN8QueryParamDTO param)
	{
		...
		if(NikonDomainConstants.RN_QRY_VAL_LVL_0.equals(param.getRunQuery()))
		{
			...
			navCatQuery = session.getNamedQuery(namedQueryToUse);
		...
		if(navCatQuery != null)
		{
			...
			navProductDetails = (List<ProductDTO>)filterUnwantedLocales(navCatQuery.setResultTransformer(new AliasToBeanConstructorResultTransformer(NikonHBN8DAOManagerTxn.ProductDTOConstructorCache)).list(), new LocaleComparator(possibleLocales), ProductDTO.class);
			
			removeLocaleOverrideProducts(session,navProductDetails, param.getLanguageCountryCode(), param.getCategory());
		...
		//TODO
		//Way may need to assemble here. Seems to work at the moment as is, so may just leave it alone
	...
	public List<ProductDTO> listCatalogueProducts(HBN8QueryParamDTO param)
	{
		...
		//Setup the session
		log.debug("Begin HBN8 Transaction");
		List<ProductDTO> catProductDetails = null;
        Session session = sessionFactory.openSession();
        try
        {
            Transaction tx = session.beginTransaction();
		...
		catQuery = session.getNamedQuery(namedQueryToUse);
		...
		catProductDetails = (List<ProductDTO>)filterUnwantedLocales(catQuery.setResultTransformer(new AliasToBeanConstructorResultTransformer(NikonHBN8DAOManagerTxn.ProductDTOConstructorCache)).list(), new LocaleComparator(possibleLocales), ProductDTO.class);
		
		removeLocaleOverrideProducts(session,catProductDetails, param.getLanguageCountryCode(), param.getCategory());
	...
	{
		enableDisableFilter(getSession(), filterName, state);
	}
	
	//Method to enable/disable a HBN8 filter
	private void enableDisableFilter(Session session, String filterName, boolean state)
	{
		if(state)
		{
	...
	private void removeLocaleOptOutProducts(List<ProductDTO> productsList) {
		log.debug("Looping through the productsList list removing any producats marked as locale opt-out...");
		for (Iterator<ProductDTO> iterator = productsList.iterator(); iterator.hasNext();) {
			ProductDTO productDTO = (ProductDTO) iterator.next();
			
			log.debug("Locale Opt Out: " + productDTO.getProdId() + " - " + productDTO.getProdLocaleOptOut());
		...
		Session session = sessionFactory.openSession();
			try
			{
				...
				if (queryList != null && queryList.size() > 0){
					...
					return queryList.get(0).getPath();
				}else{
					log.error("Cannot found DCR path for " + param.getProductId() + " in locales even using fallback");
					return "";
				}
			}catch(Exception e){
				log.error("Error while trying to get DCR path", e);
			}
		return "";
	}
