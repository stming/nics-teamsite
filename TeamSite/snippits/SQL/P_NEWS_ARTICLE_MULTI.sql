USE []
GO
/****** Object:  StoredProcedure [dbo].[news_article_mutiple]    Script Date: 04/29/2010 16:38:33 ******/
SET ANSI_NULLS ON
GO
...
-- Description: stored procedure for multiple news articles
-- =============================================
-- Note:
...
if (@SelectToDo = 'New') --AND @HonorKeepOnPage = 1)
BEGIN
...
--if (@SelectToDo = 'New' AND @HonorKeepOnPage = 0)
--BEGIN
--SELECT TOP(@numberArticles) path
--FROM         dbo.metadata
--WHERE  (DateMade >= DATEADD(mm, -3 + DATEDIFF(mm, GETDATE(), getDate()), getDate()))
