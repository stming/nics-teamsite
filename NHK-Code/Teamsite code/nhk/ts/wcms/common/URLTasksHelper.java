...
public class URLTasksHelper {

    private static Logger log = new Logger(LogFactory.getLog("nhk.ts.wcms.common.URLTasksHelper"));
    ...
    public static CSAreaRelativePath[] addDependentFiles(CSSimpleFile file, CSExternalTask currentTask, CSClient client) throws CSException {
        ...
        try {
            if (FileTypeChecker.isPageFile(file, client) || FileTypeChecker.isDcr(file) || FileTypeChecker.isHtmlFile(file)) {
                log.createLogDebug("file " + file.getName() + " is a valid page or dcr or html!!");
    ...
    private static void getDependentFiles(Vector<CSAreaRelativePath> dependentFiles, CSSimpleFile file, String workarea, CSExternalTask currentTask) {
        ...
        if (file.isValid()) {
            try {
                ...
                String[] extns = IOHelper.getCommonPropertyValue("AttachDependencyTask.fileExtensions").split(Constants.delimiter);

                log.createLogDebug(extns.length + " extns matches found.");

                String filePath = "";
                for (String extMatch : extns) {
                    log.createLogDebug("Starting with extn:::" + extMatch);
                    ...
                    while (matcher.find()) {
                        log.createLogDebug("Matcher found a match.");
                        try {
                            filePath = matcher.group();
                            if (filePath.startsWith(Constants.urlPathSep)) {
                                log.createLogDebug("$URL path before=" + filePath);
                                filePath = filePath.substring(11, filePath.length());
                                log.createLogDebug("$URL path  after=" + filePath);
                            }
                            if (filePath.startsWith(Constants.filePathSep)) {
                                log.createLogDebug("/path before=" + filePath);
                                filePath = filePath.substring(1, filePath.length());
                                log.createLogDebug("/path  after=" + filePath);
                            }
                            ...
                            log.createLogDebug("final file path is:::" + filePath);
                            ...
                            if (depfile instanceof CSSimpleFile) {
                                ...
                                } else if (FileTypeChecker.isHtmlFile(depFile)) {
                                    log.createLogDebug("depFile called by name::" + depFile.getName());
                                    getDependentFiles(dependentFiles, depFile, depFile.getArea().getName(), currentTask);
                                }
