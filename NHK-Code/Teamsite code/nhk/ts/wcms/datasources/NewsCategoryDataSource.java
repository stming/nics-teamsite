 ...
 * @author sbhojnag
 */
public class NewsCategoryDataSource extends AbstractDataSource implements MapDataSource {

    private Logger mLogger = new Logger(LogFactory.getLog("nhk.ts.wcms.datasources.NewsCategoryDataSource"));
    ...
    public Map<String, String> execute(DataSourceContext context) {
        ...
        if ((null != vpath) && (!("".equals(vpath)))) {
            try {
                ...
                propertyFullPath = propertyFullPath.substring(0, propertyFullPath.lastIndexOf("/news")) + "/SiteInfo.xml";
                ...
                Node node = propertyFileDocument.selectSingleNode("//entry[@key='pressNodeLabel']");
                System.out.println(node.getText());
                String PressRoomLabel = node.getText();
                System.out.println("PressRoomLabel: "+ PressRoomLabel);
                mLogger.createLogDebug("PressRoomLabel: " + PressRoomLabel);
